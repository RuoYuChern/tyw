    ...
    <a-row class="kline-row">
        <a-col span="8">
            <div id="amountChart" v-if="amountSeries.length > 0">
                <apexchart type="bar" height="300" :options="chartAmountOpt" :series="amountSeries"></apexchart>
            </div>
            <div id="amountChart" v-else>No Data</div>        
        </a-col>
        <a-col span="8">
            <div id="countChart" v-if="countSeries.length > 0">
                <apexchart type="bar" height="300" :options="chartCountOpt" :series="countSeries"></apexchart>
            </div>
            <div id="countChart" v-else>No Data</div>        
        </a-col> 
        <a-col span="8">
            <div id="pireChart" v-if="pieSeries.length > 0">
                <apexchart type="pie" height="300" :options="chartPieOpt" :series="pieSeries"></apexchart>
            </div>
            <div id="pireChart" v-else>No Data</div>        
        </a-col>                           
    </a-row>
    <a-table :dataSource="dataSource" :columns="columns" :pagination="pagination" size="small" class="data-row">
...
export default defineComponent({
    ...
    setup(){
        ...
        const pagination = {pageSizeOptions: ['10']}
        const columns = [
            {
            title: '股票',
            dataIndex: 'name',
    ...
    created() {
        ...
        HttpGet(apiUrl, (rsp:any)=>{
            ...
            const countLables:string [] = ["总订单","下单中","持仓","盈利","亏损","取消"]
            countData.data.push(rsp.vol)
            countData.data.push(rsp.vol - (rsp.success + rsp.failed + rsp.cancel + rsp.buy))
            countData.data.push(rsp.buy)
            ...
            const pieLables:string [] = ["下单中","持仓","盈利","亏损","取消"]
            this.chartPieOpt.labels = pieLables
            const pieData:Number[] = []
            pieData.push(rsp.vol - (rsp.success + rsp.failed + rsp.cancel + rsp.buy))
            pieData.push(rsp.buy)
